...
export async function generateCreativeImage(prompt:string,aspectRatio="1:1"){
 const res=await fetch(`${FUNCTIONS_URL}/designly-image`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${ANON_KEY}`,apikey:ANON_KEY},body:JSON.stringify({prompt,aspectRatio})});
