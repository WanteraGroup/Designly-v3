...
export async function generateCreativeVideo(prompt:string,duration:number,aspectRatio="16:9"){
 const res=await fetch(`${FUNCTIONS_URL}/designly-video`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${ANON_KEY}`,apikey:ANON_KEY},body:JSON.stringify({prompt,duration,aspectRatio})});
