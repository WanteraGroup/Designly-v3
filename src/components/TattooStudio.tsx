 ...
 return <section className='space-y-5'>
  ...
  <div className='grid gap-4 lg:grid-cols-2'>
   <div className='space-y-4 rounded-2xl border border-line bg-canvas/50 p-4'>
    ...
    <div><div className='mb-2 text-xs text-ink-400'>{t(language,'guides')}</div><div className='flex flex-wrap gap-2'>{[[grid,'grid'],[center,'center'],[mirror,'mirror']].map(([on,k])=><button key={k} type='button' onClick={()=>k==='grid'?setGrid(!grid):k==='center'?setCenter(!center):setMirror(!mirror)} className={'rounded-full border px-3 py-1.5 text-xs '+(on?'border-accent/70 bg-accent/15 text-accent':'border-line text-ink-300')}>{t(language,k as string)}</button>)}</div></div>
